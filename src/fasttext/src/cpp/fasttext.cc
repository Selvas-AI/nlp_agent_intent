...
bool FastText::quantize(std::shared_ptr<Args> qargs) {
  if (qargs->output.empty()) {
      std::cerr<<"No model provided!"<<std::endl;
      return false;
  }
  if (loadModel(qargs->output + ".bin") == false) {
    return false;
  }
...
bool FastText::test(const std::string& path, const int k, int* n, 
                    real* precision, real* recall) {
  if ((n == NULL) || (precision == NULL) || (recall == NULL)) {
    return false;
  }
  ...
  (*n) = nexamples;
  (*precision) = score / (k * nexamples);
  (*recall) = score / nlabels;
  ifs.close();
  return true;
}
...
bool FastText::predict(const int32_t k, std::istream& query,
  ...
  predictions->clear();
  if (dict_->getLine(query, model_->rng, &words) == false) {
    return false;
  }
  ...
  for (auto it = modelPredictions.cbegin(); it != modelPredictions.cend(); it++) {
    predictions->push_back(std::make_pair(dict_->getLabel(it->second), exp(it->first)));
  }
  return true;
}
...
bool FastText::wordVectors(const std::string& query, RealVector* result) {
  if (result == NULL) {
    return false;
  }
...
bool FastText::sentenceVectors(std::istream& query, RealVector* result) {
  if (result == NULL) {
    return false;
  }
  ...
  svec.zero();
  int32_t count = 0;
  while (dict_->readWord(query, word) == true) {
    getVector(vec, word);
    real norm = vec.norm();
    if (norm > 0) {
      vec.mul(1.0 / norm);
...
bool FastText::ngramVectors(const std::string& query, 
                            RealVectorResultList* results) {
  if (results == NULL) {
    return false;
  }
  ...
  std::vector<std::string> substrings;
  Vector vec(args_->dim);
  dict_->getSubwords(query, ngrams, substrings);
  for (int32_t i = 0; i < ngrams.size(); i++) {
    vec.zero();
    if (ngrams[i] >= 0) {
      vec.addRow(*input_, ngrams[i]);
    ...
    for (size_t j = 0; j < vec.size(); j++) {
      vector.push_back(vec[j]);
    }

    results->push_back(RealVectorResult(substrings[i], vector));
  }
...
bool FastText::textVectors(std::istream& query, RealVector* result) {
  if (result == NULL) {
    return false;
  }
  ...
  if (dict_->getLine(query, model_->rng, &line) == false) {
    return false;
  }
...
bool FastText::printWordVectors(const std::string& query, RealVector* result) {
  if (result == NULL) {
    return false;
  }

  if (wordVectors(query, result) == false) {
    return false;
  }
  return true;
}
...
bool FastText::printSentenceVectors(std::istream& query,
                                    RealVector* result) {
  if (result == NULL) {
    return false;
  }
  ...
  if (args_->model == model_name::sup) {
    if (textVectors(query, result) == false) {
      return false;
    }
  } else {
    if (sentenceVectors(query, result) == false) {
      return false;
    }
  }
...
bool FastText::findNN(const Matrix& wordVectors, const Vector& queryVec, 
  ...
  while (i < k && heap.size() > 0) {
    auto it = banSet.find(heap.top().second);
    if (it == banSet.end()) {
      D->push_back(Result(heap.top().second, heap.top().first));
      i++;
    }
    heap.pop();
  }
...
bool FastText::nn(const int32_t k, const std::string& queryWord, ResultList* D) {
  if (D == NULL) {
    return false;
  }
  ...
  if (findNN(wordVectors, queryVec, k, banSet, D) == false) {
    return false;
  }
...
bool FastText::analogies(const int32_t k, const std::string& A, 
  ...
  if (findNN(wordVectors, query, k, banSet, D) == false) {
    return false;
  }
...
bool FastText::loadVectors(std::string filename) {
...
bool FastText::train(std::shared_ptr<Args> args) {
  args_ = args;
  dict_ = std::make_shared<Dictionary>(args_);
  if (args_->input == "-") {
    // manage expectations
  ...
  bool result = dict_->readFromFile(ifs);
  ifs.close();
  ...
  if (args_->pretrainedVectors.size() != 0) {
    if (loadVectors(args_->pretrainedVectors) == false) {
      return false;
    }
  } else {
    input_ = std::make_shared<Matrix>(dict_->nwords()+args_->bucket, args_->dim);
