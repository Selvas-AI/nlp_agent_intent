...
bool Dictionary::readFromFile(std::istream& in) {
  std::string word;
...
bool Dictionary::getLine(std::istream& line, std::minstd_rand& rng,
                         std::vector<int32_t>* words) {
  if (words == NULL) {
    return false;
  }
  ...
  std::string word;
  int32_t ntokens = 0;
  while (readWord(line, word) == true) {
    int32_t h = find(word);
    int32_t wid = word2int_[h];
    if (wid < 0) {
      entry_type type = getType(word);
      if (type == entry_type::word) {
        word_hashes.push_back(hash(word));
      }
      continue;
    }
    entry_type type = getType(wid);
    ...
    if (ntokens > MAX_LINE_SIZE && args_->model != model_name::sup) {
      break;
    }
  }
